...
@php
    ...
    $tone = match (true) {
        in_array($normalized, ['published', 'active', 'completed', 'hadir'], true) => 'bg-emerald-50 text-emerald-700 ring-emerald-200',
        in_array($normalized, ['draft', 'planned', 'inactive'], true) => 'bg-zinc-100 text-zinc-700 ring-zinc-200',
        in_array($normalized, ['in_progress', 'dinas luar', 'announcement'], true) => 'bg-amber-50 text-amber-800 ring-amber-200',
...
<span {{ $attributes->class("inline-flex max-w-full items-center rounded-full px-2.5 py-1 text-xs font-bold ring-1 ring-inset {$tone}") }}>
    {{ $label }}
</span>
