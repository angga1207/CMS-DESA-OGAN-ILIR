...
<section class="rounded-lg border border-zinc-200 bg-white shadow-sm">
    <div class="flex flex-col gap-3 border-b border-zinc-200 p-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="font-black">Daftar Halaman</h2>
                    ...
                    <tr>
                        <td class="px-5 py-4">
                            <div class="flex min-w-64 items-center gap-3">
                                <div class="grid h-14 w-20 shrink-0 place-items-center overflow-hidden rounded-md border border-zinc-200 bg-zinc-100 text-zinc-400">
                                    @if($pageItem['featured_image_url'])
                                        <img src="{{ $pageItem['featured_image_url'] }}" alt="" loading="lazy" class="h-full w-full object-cover">
                                    @else
                                ...
                                <div>
                                    ...
                                    <div class="mt-1 line-clamp-1 text-xs text-zinc-500">{{ $pageItem['excerpt'] ?: 'Belum ada ringkasan halaman.' }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-5 py-4 text-zinc-500">{{ $pageItem['slug'] }}</td>
                        <td class="px-5 py-4"><x-admin.pill :value="$pageItem['status']" /></td>
                        <td class="px-5 py-4">
                            <div class="flex gap-2">
                                <a href="{{ route('admin.pages.edit', $pageItem['id']) }}" class="rounded bg-zinc-100 px-3 py-2 text-xs font-bold">Edit</a>
                                <button wire:click="delete({{ $pageItem['id'] }})" wire:confirm="Hapus halaman ini?" class="rounded bg-red-50 px-3 py-2 text-xs font-bold text-red-700">Hapus</button>
                            </div>
                        </td>
                    </tr>
                @endforeach
    ...
    <div class="flex items-center justify-between gap-4 border-t border-zinc-200 px-5 py-4 text-sm">
        ...
        <div class="flex gap-2">
            ...
            <button type="button" wire:click="nextPage" @disabled($page >= max((int) ceil($totalPages / $perPage), 1)) class="rounded-md border border-zinc-200 px-3 py-2 font-bold disabled:opacity-40">Berikutnya</button>
        </div>
    </div>
</section>
